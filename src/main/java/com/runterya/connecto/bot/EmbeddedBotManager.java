    ...
    private static void runBotLoop(int port, String botName) {
        while (RUNNING.get()) {
            try {
                ...
                currentSocket.setSoTimeout(0); // Infinite read timeout
                ...
                ConnectoMod.LOGGER.info("[Connecto] ✓ Embedded TCP bot connected successfully! Active TCP socket open on 127.0.0.1:{}.", port);

                // 3. Continuously drain incoming server packets to keep TCP window open
                byte[] buffer = new byte[2048];
                while (RUNNING.get() && !currentSocket.isClosed()) {
                    int bytesRead = in.read(buffer);
                    if (bytesRead == -1) {
                        ConnectoMod.LOGGER.info("[Connecto] Embedded TCP bot connection closed by server.");
                        break;
                    }
                }
                ...
                if (RUNNING.get()) {
                    ConnectoMod.LOGGER.warn("[Connecto] Embedded TCP bot socket closed ({}). Reconnecting in 5s...", e.getMessage());
                }
            } finally {
                if (currentSocket != null) {
